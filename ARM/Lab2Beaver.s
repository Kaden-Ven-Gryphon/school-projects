...
prompt1:
	...
	add	r4, r4, #1			@up the counter for the prompt
...
prompt2:
	...
	addle	r4, r4, #1			@add 1 to the counter for prompts
	ble	prompt3				@go to prompt 3
...
runOperation:
	...
	bleq	functionAND			@call the AND function

	cmp	r4, #2				@if the input was a 2
	bleq	functionORR			@call the ORR function

	cmp	r4, #3				@if the input was a 3
	bleq	functionEOR			@call the EOR function

	cmp	r4, #4				@if the input was a 4
	bleq	functionBIC			@call the BIC function
...
functionBIC:											@
												@
	pop	{r2}				@get the first operand form the stack		@
												@
	pop	{r3}				@get the second operand from the stack		@
												@
	bic	r2, r2, r3			@preform the bic operation			@
												@
	push 	{r2}				@push the result onto the stack			@
												@
	bx	lr				@return						@
