...
void my_3d_projection(int width, int height)
{
	...
	width_bound = (GLfloat) width/2; height_bound = (GLfloat) height/2;						//to make (0,0) the center of the canvas
	glViewport(0, 0, width, height);
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	glOrtho(-width_bound, width_bound, -height_bound, height_bound, 0.0, 640.0);			//changed the planes
